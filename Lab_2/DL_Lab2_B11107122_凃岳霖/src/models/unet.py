...
class DoubleConv(nn.Module):
    '''
    實作 2015 原版的藍色箭頭的部分，包含兩層 3x3 卷積和 ReLU 激活函數 (conv 3x3, ReLU)*2 。
    '''
    ...
    def __init__(self, in_channels, out_channels):
        ...
        *** 2015 原版的卷積層沒有 padding，因此每經過一次 3x3 卷積，長寬都會減少 2。
        '''
    ...
    # 前向傳播函數，將輸入 x 傳遞給卷積層並返回結果。
    def forward(self, x):
        return self.conv(x)
    
class UNet(nn.Module):
    '''
    實作 2015 原版的 UNet 模型的架構如下：
        - Encoder (U 左側，下采樣用):
            - 4 層卷積，每層卷積後接一個 2x2 的 Max Pooling 層，通道數分別為 64、128、256、512。
        - Decoder (U 右側，上採樣用): 
            - 4 層卷積，每層卷積前接一個 2x2 的 Up-conv 層，通道數分別為 512、256、128、64。
    ...
    def __init__(self, in_channels=3, out_channels=1, feature_channels=[64, 128, 256, 512]):
        '''
        Args:
            in_channels (int): 輸入圖像的通道數，默認為 3 (RGB 圖像)。
            out_channels (int): 輸出圖像的通道數，默認為 1 (單通道二值圖像)。
            feature_channels (list): 各層卷積的通道數列表。
        '''
        ...
        for feature in reversed(feature_channels):
            ...
            self.ups.append(DoubleConv(feature * 2, feature))                                   # 添加 DoubleConv 層。
...
if __name__ == "__main__":
    ...
    print(f"[論文原版尺寸測試] 輸入: {x.shape} -> 輸出: {output_x.shape}") # 預期輸出形狀為 (1, 1, 388, 388)。