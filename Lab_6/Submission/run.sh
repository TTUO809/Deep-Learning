...
DATA_ROOT="$SCRIPT_DIR"     # ← Set this to your data directory if file/ and iclevr/ are elsewhere
...
# Common path args shared across all evaluate.py calls
EVAL_PATH_ARGS=(
    --obj_json      "$DATA_ROOT/file/objects.json"
    --test_json     "$DATA_ROOT/file/test.json"
    --new_test_json "$DATA_ROOT/file/new_test.json"
    --out_dir       "$SCRIPT_DIR/images"
    --grid_dir      "$SCRIPT_DIR/results"
)
...
if [ "$DO_TRAIN" = true ]; then
    TRAIN_BASE_ARGS=(
        ...
        --save_every 10
        --wandb
        --img_dir    "$DATA_ROOT/iclevr"
        --train_json "$DATA_ROOT/file/train.json"
        --obj_json   "$DATA_ROOT/file/objects.json"
        --ckpt_dir   "$CKPT_DIR"
    )
    if [ ! -f "$LAST_CKPT" ]; then
...
if [ "$DO_DENOISE" = true ]; then
    ...
    python denoise_process.py \
        --ckpt "$LAST_CKPT" \
        --obj_json "$DATA_ROOT/file/objects.json" \
        --out     "$SCRIPT_DIR/results/denoise_process.png" \
        --scheduler ddpm \
        --snapshots 8
else
