...
DDIM, 100 steps, $s=3.0$                 & 0.9167 & 0.9762 \\
...
Adding classifier guidance $s=2.0$ to DDIM-100 lifts the result to the overall best:
test $0.9583$ and new\_test $0.9881$.
...
Guidance scale $s=2.0$ achieves the best combined accuracy (test $0.9583$,
new\_test $0.9881$).
Test accuracy is non-monotone with $s$: it dips at $s=3.0$ ($0.9167$) then partially
recovers at $s=4.0$ ($0.9583$), while new\_test accuracy decreases monotonically beyond
$s=2.0$.
This confirms that moderate guidance maximises condition adherence without distorting
image structure.
...
\begin{figure}[H]
  ...
  \caption{Full classifier guidance scale sweep (DDIM, 100 steps, EMA weights).
           ...
           $s=2.0$ maximises both metrics; new\_test accuracy degrades monotonically beyond $s=2$.}
...
\begin{figure}[H]
  ...
  % new_test.json
  \begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{../DL_LAB6_B11107122_凃岳霖/results/new_test_guided_s2_grid.png}
    \caption{\texttt{new\_test} -- EMA (0.9881)}
  \end{subfigure}\hfill
  \begin{subfigure}[b]{0.48\linewidth}
    \includegraphics[width=\linewidth]{../DL_LAB6_B11107122_凃岳霖/results/new_test_no_ema_grid.png}
    \caption{\texttt{new\_test} -- Raw (0.9167)}
  \end{subfigure}

  \caption{EMA vs.\ raw weights on both test splits (DDIM, 100 steps, $s=2.0$).
           EMA improves test accuracy by $+1.4$ pp and new\_test accuracy by $+7.1$ pp,
           with the gap especially pronounced on the harder \texttt{new\_test} split.}
  \label{fig:ema_cmp}
\end{figure}
